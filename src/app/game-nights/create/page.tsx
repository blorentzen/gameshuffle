...
import { listTemplates, getTemplate } from "@/lib/game-nights/templates";
import type { GameNight, NightLevel } from "@/lib/game-nights/types";
...
export default async function CreateNightPage({
  ...
  if (!user) redirect("/login?redirect=/game-nights/create");

  const { template: templateId } = await searchParams;
  const templates = await listTemplates().catch(() => []);

  // Prefill from a template (date left blank for the new night).
  let initial: GameNight | undefined;
  if (templateId) {
          ...
          <div className="bgn-templates">
            ...
            <div className="bgn-templates__row">
              {templates.map((t) => (
                <Link
                  key={t.id}
                  href={`/game-nights/create?template=${t.id}`}
              ...
              {templateId && (
                <Link href="/game-nights/create" className="bgn-template-chip">Start blank</Link>
              )}
            </div>
          </div>
        )}
