...
/**
 * Game night RSVP reminders. Once per night, ~24h out, everyone who RSVP'd
 ...
 * means no reminders fire. See `supabase/game night-reminders.sql`.
 */
...
export async function sendDueNightReminders(): Promise<{ nights: number; notifs: number; emails: number; discord: number }> {
  ...
  for (const n of ((nights ?? []) as NightRow[])) {
    ...
    const url = `${base}/game-nights/${n.id}`;
