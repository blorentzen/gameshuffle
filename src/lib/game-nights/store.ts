...
/**
 * Game night store (server-only). RLS enforces host ownership, so these
...
export async function createNight(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      // Guarded: only sent when chosen so pre-migration inserts keep working
      // (the column defaults to 'board').
...
export async function getNight(id: string): Promise<GameNight | null> {
  const supabase = await createClient();
  const { data } = await supabase
    ...
    .eq("id", id)
    .maybeSingle();
  return (data as GameNight | null) ?? null;
}

export async function listNightsForHost(hostId: string): Promise<GameNight[]> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("board_game_nights")
    .select("*")
 ...
 */
export async function listNightsAttending(userId: string): Promise<GameNight[]> {
  const supabase = await createClient();
...
export async function listPublicNights(limit = 50): Promise<GameNight[]> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("board_game_nights")
    .select("*")
 ...
 */
export async function listNightsForCommunity(communityId: string, limit = 12): Promise<GameNight[]> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("board_game_nights")
    .select("*")
