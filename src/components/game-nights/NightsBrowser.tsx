...
export function NightsBrowser({
  ...
  const rows = useMemo(() => {
    ...
    if (kind) list = list.filter((r) => (r.night.kind ?? "board") === kind);
    ...
    return list;
  }, [nights, query, genre, level, kind, sort, radius, coords, hasPrefs, viewerPrefs]);
      ...
      {forYou.length > 0 && (
        <div className="bgn-foryou">
          ...
          <div className="bgn-foryou__row">
            {forYou.map(({ night: n }) => (
              <Link key={n.id} href={`/game-nights/${n.id}`} className="bgn-foryou__card">
      ...
      <div className="bgn-filters">
        ...
        <div className="bgn-filters__field">
          ...
          <Select
            ...
            fullWidth
            options={[{ value: "", label: "Anything" }, ...NIGHT_KINDS.map((k) => ({ value: k.value, label: k.short }))]}
          />
        </div>
        <div className="bgn-filters__field">
              ...
              <Link key={n.id} href={`/game-nights/${n.id}`} className="bgn-card">
                ...
                <span className="bgn-card__body">
                  ...
                  <span className="bgn-card__tags">
                    {n.kind && n.kind !== "board" && <span className="bg-badge bg-badge--kind">{nightKindLabel(n.kind, true)}</span>}
